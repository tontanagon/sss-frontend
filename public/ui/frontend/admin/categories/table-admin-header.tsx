...
export default function TableItemAdmin() {
  ...
  const [title, setTitle] = useState("ลบหมวดหมู่สำเร็จ");
    ...
    debounceTimer.current = setTimeout(() => {
      const getItem = async () => {
        ...
        const res = await fetch(
          `/medadm/api/categories/get?limit=${search.pagination}&search_text=${search.search_text}`
        );
  ...
  const confirmDelete = async () => {
    ...
    const res = await fetch("/medadm/api/categories/delete", {
      method: "POST",
      headers: {
