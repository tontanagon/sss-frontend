...
export default function TableItemAdmin() {
  ...
  const [title, setTitle] = useState("ลบประเภทสำเร็จ");
    ...
    debounceTimer.current = setTimeout(() => {
      const getItem = async () => {
        const res = await fetch(
          `/medadm/api/types/get?limit=${search.pagination}&search_text=${search.search_text}`
        );
  ...
  const confirmDelete = async () => {
    const res = await fetch("/medadm/api/types/delete", {
      method: "POST",
      headers: {
