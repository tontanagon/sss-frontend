...
export default function ProductLoss({ serchDate = "" }: { serchDate?: Date | string }) {
  const [items, setItems] = useState<DataProductLost[]>();

  useEffect(() => {
    const getItems = async () => {
      const res = await fetch("/medadm/api/dashboard/product-lost");
  ...
  return (
    <div className="p-5 flex flex-col shadow-lg rounded-md w-full h-[360px]">
      <div className="text-lg font-medium mb-2">
        ตารางแสดงวัสดุ-อุปกรณ์ที่ไม่ถูกส่งคืน (หาย)
      </div>
        ...
        <table className="table-fixed w-full">
          ...
          <tbody>
            {items && items.length > 0 ? (
              items.map((booking: DataProductLost, index: number) => {
                return (
                  <tr
                    key={index}
