...
export default function BookingNow({ serchDate = "" }: { serchDate?: Date | string }) {
  const router = useRouter();
  const [items, setItems] = useState<BookingHistory[]>();

  useEffect(() => {
    const getItems = async () => {
      const res = await fetch("/medadm/api/dashboard/booking-with-status-now");
