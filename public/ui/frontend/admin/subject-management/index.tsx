...
export default function SubjectManagementPage() {
  ...
  const getItems = async () => {
    const res = await fetch(
      `/medadm/api/subject?limit=${search.pagination}&search_text=${search.search_text}`
    );
  ...
  const confirmDelete = async () => {
    ...
    const res = await fetch(`/medadm/api/subject/${idItem}`, {
      method: "DELETE",
      headers: {
    ...
    if (!data.status) {
      ...
      setTitle("ลบกระบวนวิชาไม่สำเร็จ");
      setIcon(faCircleXmark);
    } else {
      setError(data.message);
      setTitle("ลบกระบวนวิชา สำเร็จ");
    }
        ...
        <table className="table-fixed w-full mb-3">
          <thead className={`text-white font-medium bg-[#0055CAE5]`}>
            <tr className="md:text-[1rem] text-[0.8rem]">
              ...
              <th className="md:px-[20px] md:py-[10px] px-2 py-1 text-start md:w-50 w-15">
                รหัสวิชา
              </th>
              <th className="md:px-[20px] md:py-[10px] px-2 py-1 text-start">
                ชื่อวิชา
              </th>
              <th className="md:px-[20px] md:py-[10px] px-2 py-1 text-start md:w-30 w-10">
                สถานะ
              </th>
          ...
          <tbody>
            {items && items?.data.length > 0 ? (
              items?.data.map((subject: CoreConfigs, index: number) => {
                return (
                  <tr
                    key={index}
                  ...
                  >
                    ...
                    <td
                      className={`md:px-[20px] md:py-[10px] px-2 py-1 font-normal ${
                        subject.status === "active" ? "text-green-600" : "text-red-600"
                      }`}
                    >
                      {subject.status === "active" ? "ใช้งาน" : "ไม่ได้ใช้งาน"}
                    </td>
                    <td className="md:px-[20px] md:py-[10px] px-2 py-1">
                      <div className="flex gap-3">
                        <Link
                          href={`subject-management/edit/${subject.id}`}
                          onNavigate={() => {
